...
int main() {
	...
	// Push another scope so most memory should be freed *before* we exit the app
	{
		...
		{
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/HitBox.obj");
		...
		while (!glfwWindowShouldClose(window)) {
			...
			if (Application::Instance().ActiveScene == scene) {
				...
				{
					Application::Instance().ActiveScene = Pause;//remove later players shouldnt be able access this scene(debugging scene)
				}
